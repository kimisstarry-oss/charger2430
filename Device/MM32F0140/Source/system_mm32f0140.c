...
#define VECTOR_TABLE_COPY_ADDR  0x20000000  // 向量表复制到RAM起始地址
#define APP_START_ADDR          0x08004000  // App起始地址，必须和Keil配置的IROM1一致
...
void SystemInit (void)
{
    ...
    /* ===== 中断向量表重映射到RAM (M0内核没有VTOR) ===== */
    /* 只有在 Bootloader 中才执行此操作 */
#if defined(BUILD_BOOTLOADER)  // 新增宏定义
    uint32_t i;
    
    /* 1. 使能SYSCFG时钟 */
    RCC->APB2ENR |= (1 << 0);
    
    /* 2. 将中断向量表从Flash复制到RAM */
    for (i = 0; i < 48; i++) {  // MM32F0144有48个中断向量
        *((__IO uint32_t*)VECTOR_TABLE_COPY_ADDR + i) = 
            *(__IO uint32_t*)(APP_START_ADDR + (i << 2));
    }
    
    /* 3. 配置SYSCFG，将RAM映射到地址0x00000000 */
    SYSCFG->CFGR = (SYSCFG->CFGR & ~SYSCFG_CFGR_MEM_MODE) | (0x3 << 0);  // MEM_MODE = 0b11
#endif
}
